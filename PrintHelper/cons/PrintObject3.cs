...
{
    public partial class PrintObject3 : UserControl, IPrintObject,  ISizeable, IDeleteable,
        IFormatable, IFontable, IColorable, IChangeAreaAble, IContextAlignAble, IBorderable 
    {
        ...
        IPrintObject IPrintObject.Copy()
        {
            ...
            IPrintObject item = new PrintObject3();
            string xml = "<xml>" + ins.xml + "</xml>";
            Helper.ReadXml r = new Helper.ReadXml(xml);
            xml = "<xml>" + r.Read("PrintObject3") + "</xml>";
            item.xml = xml;
            return item;
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            e.Graphics.DrawString("sum(" + _Field +")", this.Font, new SolidBrush(this._Color), rec, sf);
