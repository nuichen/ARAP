...
{
    public partial class frmBankCashInitial : Form
    {
        public frmBankCashInitial()
        {
            InitializeComponent();
        ...
        //    }
        //}
        Page<rp_t_bank_beginbalance> page = new Page<rp_t_bank_beginbalance>();
       private DataTable data_table = new DataTable();

        private void LoadDataGrid()
        {        
            this.dgvSup.AddColumn("visa_id", "现金银行编号", "", 150, 1, "",false);
            this.dgvSup.AddColumn("delete_flag", "删除", "", 50, 2, "", false);
            this.dgvSup.AddColumn("visa_nm", "现金银行名称", "", 150, 2, "", false);
            ...
            this.dgvSup.DataSource = page.Assign<rp_t_bank_beginbalance>();
        }
        ...
        private void LoadSup()
        {
            ...
            Thread th = new Thread(() =>
            {
                ...
                try
                {
                    ...
                    //{
                    //IBLL.ISCPaymentBLL bll = new BLL.SCPaymentBLL();
                    page = bll.GetBankCashBeginbalance(
                              Keyword,
                              page
                              );
                    //}
        ...
        private void tsbSave_Click(object sender, EventArgs e)
        {
            ...
            List<rp_t_bank_beginbalance> list = new List<rp_t_bank_beginbalance>();
            try
            {
                foreach (DataRow supcust in tb.Rows)
                {
                    rp_t_bank_beginbalance temp = new rp_t_bank_beginbalance();
                    temp.visa_id = Convert.ToString(supcust["visa_id"]);
                ...
                //if (_runType == 0)
                //{
                    bll.SavaBankCashBeginbalance(list, Program.oper.oper_id);
                //}
                //else if (_runType == 1)
                //{
            ...
            if (YesNoForm.ShowFrom("确认要删除选中吗?") == DialogResult.Yes)
            {
                ...
                foreach(DataRow dr in dt.Rows)
                {
                    if (Conv.ToString(dr["delete_flag"]) == "1")
                    {
                        if (is_one == false)
                        {
                            sup_no += "'" + Conv.ToString(dr["visa_id"]) + "'";
                            is_one = true;
                            lr.Add(dr);
                        }
                        else
                ...
                bll.DeleteBankCashBeginbalance(sup_no, DateTime.Now);
