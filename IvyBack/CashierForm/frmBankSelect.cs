...
{
    public partial class frmBankSelect : Form
    {
        public frmBankSelect()
        {
            InitializeComponent();
        ...
        {
            this.dgvSup.AddColumn("visa_id", "现金银行编号", "", 150, 1, "");
            //this.dgvSup.AddColumn("delete_flag", "删除", "", 50, 2, "");
            this.dgvSup.AddColumn("visa_nm", "现金银行名称", "", 150, 1, "");
           // this.dgvSup.AddColumn("pay_kind", "借贷", "", 150, 2, "{0:平,1:借,2:贷}");
            //this.dgvSup.AddColumn("begin_balance", "期初金额", "", 150, 1, "");
        ...
        private void frmSupcust_Load(object sender, EventArgs e)
        {
            ...
            {
                MsgForm.ShowFrom(ex);
                LogHelper.writeLog("frmBankSelect=>load", ex.ToString());
            }
            finally
            {
