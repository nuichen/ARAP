...
namespace IvyBack.cons
{
    public class DataGrid : System.Windows.Forms.Control
    {
        ...
        private bool _MergeCell = false;
        private ContextMenuStrip cmsMain;
        private ToolStripMenuItem tsmiAllSelect;
        private ToolStripMenuItem tsmiAllUnSelect;
        ...
        public DataTable DataSource
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (_IsSelect)
                    {
                        if (!tb_source.Columns.Contains("select_flag"))
                        {
                            tb_source.Columns.Add("select_flag");
                        }
                        if (!tb.Columns.Contains("select_flag"))
                        {
                            tb.Columns.Add("select_flag");
                        }

                        if (lstcolumn.FirstOrDefault(c => c.column_name.Equals("select_flag")) == null)
                        {
                            ColumnInfo col = new ColumnInfo
                            {
                                column_name = "select_flag",
                                header = "选择",
                                group_header = "",
                                width = 60,
                                align = 2,
        ...
        private void InitializeComponent()
        {
            ...
            this.tsmiAllUnSelect = new System.Windows.Forms.ToolStripMenuItem();
            ...
            this.tsmiAllSelect.Size = new System.Drawing.Size(112, 22);
            this.tsmiAllSelect.Tag = "select";
            ...
            // tsmiAllUnSelect
            // 
            this.tsmiAllUnSelect.Name = "tsmiAllUnSelect";
            this.tsmiAllUnSelect.Size = new System.Drawing.Size(112, 22);
            this.tsmiAllUnSelect.Tag = "select";
            this.tsmiAllUnSelect.Text = "全反选";
            this.tsmiAllUnSelect.Visible = false;
            this.tsmiAllUnSelect.Click += new System.EventHandler(this.tsmiAllUnSelect_Click);
            // 
            // DataGrid
            // 
                        ...
                        if (is_group == false)
                        {
                            if (MergeCell == true)
                            {
                                ...
                                for (int i = 0; i < tb.Rows.Count; i++)
                                {
                                    ...
                                    if (i % 2 == 1)
                                    {
                                        back_color = GlobalData.grid_second;
                                    }
                                    ...
                                    for (int j = 0; j < lstcolumn.Count; j++)
                                    {
                                        ...
                                        else if (cur_cell.row_index == i)
                                        {
                                            ...
                                            if (cur_cell.column_index == j)
                                            {
                                                b = GlobalData.grid_sel_cell;
                                            }
                                        ...
                                        if (tb.Columns.Contains(col.column_name) == true)
                                        {
                                            if (j == 0)
                                            {
                                                ...
                                                DrawCellBorder(g, x, y, w, h, last_row);
        ...
        */
        ...
        private CellObject cur_cell
        {
            ...
            set
            {
                if (_cur_cell == null)
                {
                    ...
                    {
                        var col = lstcolumn[value.column_index];
                        CurrentRowChange?.Invoke(this, col.column_name, tb.Rows[value.row_index]);
                        CurrentCellChange?.Invoke(this, col.column_name, tb.Rows[value.row_index]);
                        _cur_cell = value;
                    }
                }
                else if (value != null)
                {
                    ...
                    {
                        var col = lstcolumn[value.column_index];
                        if (cur_cell.row_index != value.row_index)
                        {
                            CurrentRowChange?.Invoke(this, col.column_name, tb.Rows[value.row_index]);
                            CurrentCellChange?.Invoke(this, col.column_name, tb.Rows[value.row_index]);
                        }
                        else if (cur_cell.column_index != value.column_index)
                        {
                            CurrentCellChange?.Invoke(this, col.column_name, tb.Rows[value.row_index]);
                        }
        ...
        public delegate void CurrentRowChangeHandler(object sender, string column_name, DataRow row);
        public event CurrentCellChangeHandler CurrentCellChange;
        public event CurrentRowChangeHandler CurrentRowChange;
        ...
        public delegate void SelectRowChangeHandler(object sender, string column_name, DataRow row, MouseEventArgs e);
        /// <summary>
        /// 点击单元格
        /// </summary>
        ...
        public event SelectRowChangeHandler SelectRowChange;
        ...
        public int CurrentRowIndex()
        {
            ...
            {
                return cur_cell.row_index;
            }
        }
        public void SetRowIndex(int row_index, int column_index = 1)
        {
            if (row_index < 0)
            {
                row_index = 0;
            ...
            if (row_index > tb.Rows.Count - 1)
            {
                row_index = tb.Rows.Count - 1;
            }
            foreach (CellObject obj in lstobj)
            {
                if (obj.type == 0)
                {
                    if (obj.column_index == column_index)
                    {
                        if (obj.row_index == row_index)
                        {
                            this.cur_cell = obj;
                            Refresh();
                            break;
        ...
        public void SimDoubleClick()
        {
            ...
            {
                me = new MouseEventArgs(MouseButtons.Left, 1, cur_cell.r.Left + 1, cur_cell.r.Top + 1, 0);
                dr = tb.Rows[cur_cell.row_index];
            }

            DoubleClickCell.Invoke(this, lstcolumn[0].column_name, dr, me);
        }
        ...
        public DataTable GetSelectData()
        {
            ...
            DataTable tb = this.tb.Clone();

            foreach (DataRow dr in this.tb.Rows)
            {
                if (dr["select_flag"].ToString().Equals("1"))
                {
                    tb.ImportRow(dr);
                }
            }
        ...
        public List<DataRow> GetSelectDatas()
        {
            if (!IsSelect) return null;
        ...
        public DataTable GetNoSelectData()
        {
            if (!IsSelect) return null;
        ...
        public List<DataRow> GetNoSelectDatas()
        {
            if (!IsSelect) return null;
        ...
        private void TargetRowSelect(CellObject obj)
        {
            if (lstcolumn[obj.column_index].column_name.Equals("select_flag"))
            {
                tb.Rows[obj.row_index]["select_flag"] = tb.Rows[obj.row_index]["select_flag"].ToString().Equals("1") ? "0" : "1";
                SelectRowChange?.Invoke(this, "select_flag", tb.Rows[obj.row_index], null);
            }
        }
        public void RowAllSelect()
        {
            foreach (DataRow dr in this.tb.Rows)
            {
                dr["select_flag"] = "1";
        ...
        public void RowAllSelect(string coloumn, string value)
        {
            foreach (DataRow dr in this.tb.Rows)
            {
                if (value.Equals(dr[coloumn].ToString()))
                {
                    dr["select_flag"] = "1";
                }
            }
        ...
        private void DrawCellBorder(Graphics g, int x, int y, int w, int h, bool last_row)
        {
            if (MergeCell == false)
            {
                g.DrawLine(Pens.Gray, x, y, x, y + h);
        ...
        private void ChangeCms(string tag, bool flag)
        {
            foreach (ToolStripItem item in this.cmsMain.Items)
            {
                if (item.Tag != null && tag.Equals(item.Tag.ToString()))
                {
                    item.Visible = flag;
                }
            }
        ...
        private void tsmiAllUnSelect_Click(object sender, EventArgs e)
        {
            RowUnSelect();
        }
