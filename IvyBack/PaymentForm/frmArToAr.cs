...
{
    public partial class frmArToAr : Form, IOrder
    {
            ...
            try
            {
                ...
                System.Threading.Thread th = new System.Threading.Thread(() =>
                {
                    ...
                    try
                    {
                        ...
                        //var sup = paymentbll.GetSupcustList("C");
                        this.txtcus.Invoke((MethodInvoker)delegate
                        {
                            this.txtsup.Bind(bll.GetDataTable("", "", 1, 1, 20000, out tmp), 300, 200, "supcust_no", "supcust_no:客户编码:100,sup_name:客户名称:150", "supcust_no/sup_name->Text");
                        });
        ...
        void IOrder.Save()
        {
            try
            {
                ...
                {
                    throw new Exception("转入客户必填!");
                }
                if (txtsup.Text.Trim().Contains("/") == false)
                {
                    throw new Exception("转出客户必填!");
                }
        ...
        public void GetPrintTb()
        {
...

            {
                Dictionary<string, string> file_dic = new Dictionary<string, string>()
                {
                    {"sheet_no","单据号"},
                    {"supcust_from","转入客户编号"},
                    {"supcust_from_name","转入客户名称"},
                    {"supcust_to","转出客户编号"},
                    {"supcust_to_name","转出客户名称"},
                    {"total_amount","对冲金额"},
                    {"oper_id","操作人编号"},
                    {"oper_name","制单人"},
                    {"oper_date","操作日期"},
