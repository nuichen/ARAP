...
{
    public partial class frmCusCollectionARAP : Form, IOrder
    {
        ...
        public frmCusCollectionARAP(int run)
        {
                    ...
                    try
                    {
                        ...
                        if (runType1 == 1)
                        {
                           // cus = paymentbll.GetSupcustList("C");
                            ...
                            {
                                this.txtcus.Bind(bll.GetDataTable("", "", 1, 1, 20000, out tmp), 300, 200, "supcust_no", "supcust_no:客户编码:100,sup_name:客户名称:150", "supcust_no/sup_name->Text");
                            });
                        }
                        else
                        {
                            ...
                            this.txtcus.Invoke((MethodInvoker)delegate
                            {
                                this.txtcus.Bind(bll1.GetDataTable("", "", 1, 1, 20000, out tmp), 300, 200, "supcust_no", "supcust_no:供应商编码:100,sup_name:供应商名称:150", "supcust_no/sup_name->Text");
                            });
                        }
        ...
        void IOrder.Save()
        {
            try
            {
                ...
                if (sum != 0)
                {
                    MsgForm.ShowFrom("本次核销金额的累加值必须为0");
                    return;
                }
                if (runType == 1)
                {
       ...
       public bool isAll = false;
                ...
                System.Threading.Thread th = new System.Threading.Thread(() =>
                {
                    ...
                    try
                    {
                        ...
                        this.Invoke((MethodInvoker)delegate
                        {
                            ...
                            //{
                            //    if ((Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["paid_amount"]) < 0))
                            ...
                            //txtCumulative.Text = Conv.ToString(num_ber);
                            this.editGrid1.Refresh();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (dt.Rows[i]["select_flag"].ToString() == "1" && Conv.ToDecimal(dt.Rows[i]["yf_amount"]) > 0)
                {                 
                    cum1 += Conv.ToDecimal(dt.Rows[i]["yf_amount"]);
            ...
            if (cum >= cum1)
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {                     
                        ...
                        dt.Rows[i]["num1"] = dt.Rows[i]["yf_amount"].ToDecimal() - dt.Rows[i]["pay_amount"].ToDecimal();

                    }
                    else if (dt.Rows[i]["select_flag"].ToString() == "1" && Conv.ToDecimal(dt.Rows[i]["yf_amount"]) < 0)
                    {
                        if (cum1 >= Math.Abs(Conv.ToDecimal(dt.Rows[i]["yf_amount"])))
                        {
                            dt.Rows[i]["pay_amount"] = dt.Rows[i]["yf_amount"];
                            dt.Rows[i]["num1"] = dt.Rows[i]["yf_amount"].ToDecimal() - dt.Rows[i]["pay_amount"].ToDecimal();
                            cum1 += Conv.ToDecimal(dt.Rows[i]["yf_amount"]);
            ...
            else
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    {
                        if (cum >= Math.Abs(Conv.ToDecimal(dt.Rows[i]["yf_amount"])))
                        {
                            dt.Rows[i]["pay_amount"] = dt.Rows[i]["yf_amount"];
                            dt.Rows[i]["num1"] = dt.Rows[i]["yf_amount"].ToDecimal() - dt.Rows[i]["pay_amount"].ToDecimal();
                            cum -= Conv.ToDecimal(dt.Rows[i]["yf_amount"]);
                        ...
                        {
                            dt.Rows[i]["pay_amount"] = Math.Abs(cum);
                            dt.Rows[i]["num1"] = dt.Rows[i]["yf_amount"].ToDecimal() - dt.Rows[i]["pay_amount"].ToDecimal();
                            cum = 0;
                        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (isAll)
            {
                MsgForm.ShowFrom("已经生成全部数据，无需再手动添加。");
                return;
            }
            if (string.IsNullOrEmpty(this.txtcus.Text)) return;
            //if (!string.IsNullOrEmpty(this.txtsheet_no.Text)) return;
            ...
            if (frm.ShowPayment(out list) == DialogResult.Yes)
            {
                ...
                //foreach (DataRow dr in tb4.Rows)
                //{
                //    if ((Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"]) < 0))
                //        num_ber += Math.Abs((Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"])));
                //}
                ////if (runType1 == 1)
                ////    foreach (DataRow dr in tb4.Rows)
                ////    {
                ////        if (Conv.ToString(dr["voucher_first"]) == "CP" && (Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"]) < 0))
                ////            num_ber += Math.Abs((Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"])));
                ////    }
                ////else
                ////{
                ////    foreach (DataRow dr in tb4.Rows)
                ////    {
                ////        if (Conv.ToString(dr["voucher_first"]) == "RP" && (Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"]) < 0))
                ////            num_ber += Math.Abs((Conv.ToDecimal(dr["sheet_amount"]) - Conv.ToDecimal(dr["已核销金额"])));
                ////    }
                ////}
