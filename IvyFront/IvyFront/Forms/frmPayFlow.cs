...
{
    public partial class frmPayFlow : Form
    {
        private int pageSize = 10;
        private int pageIndex = 1;
        private int total = 0;
        private int pageCount = 0;
        private List<Model.ot_pay_flow> all_list = new List<Model.ot_pay_flow>();
        public frmPayFlow(DateTime date1,DateTime date2,string order_key)
        {
            InitializeComponent();
            //设置表格
        ...
        private void pageChanged()
        {
            try
            {
                ...
                all_list = bll.GetPayFlow(pnldate1.Text, pnldate2.Text, order_key, pageIndex, pageSize, out total);
                foreach (Model.ot_pay_flow item in all_list)
                {
                    ...
                    row.Cells[2].Value = item.sup_name;
                    row.Cells[3].Value = item.pay_way_str;
                    row.Cells[4].Value = item.sale_amount.ToString("F2");
                    row.Cells[5].Value = item.pay_amount.ToString("F2");
        ...
        private void frmPayFlow_Click(object sender, EventArgs e)
        {
            pnlboard.Visible = false;
        }
        ...
        private void frmPayFlow_FormClosed(object sender, FormClosedEventArgs e)
        {
            all_list = null;
        }
