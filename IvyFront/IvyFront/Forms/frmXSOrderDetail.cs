...
{
    public partial class frmXSOrderDetail : Form
    {
        ...
        public frmXSOrderDetail(string sheet_no)
        {
            ...
            lbl_total_amt.Text = all_dt.Compute("sum(sale_money)", "").ToString();
            ...
            try
            {
                ...
                for (int i = (pageIndex - 1) * pageSize; i < pageIndex * pageSize; i++) 
                {
                    if (i < all_dt.Rows.Count) 
                    {
                        ...
                        row.Cells[4].Value = all_dt.Rows[i]["sale_qnty"].ToString();
                        row.Cells[5].Value = all_dt.Rows[i]["real_price"].ToString();
                        row.Cells[6].Value = all_dt.Rows[i]["sale_money"].ToString();
                    }
                }
