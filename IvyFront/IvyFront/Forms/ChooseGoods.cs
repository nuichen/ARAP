...
{
    public partial class ChooseGoods : Form
    {
        List<Model.bi_t_item_info> goodsList = new List<Model.bi_t_item_info>();
        List<Model.bi_t_item_info> goodsListAll = new List<Model.bi_t_item_info>();

        public ChooseGoods(List<bi_t_item_info> goodsList)
        {
            InitializeComponent();
            //
            ...
            try
            {
                foreach (Model.bi_t_item_info item in goodsList)
                {
                    goodsListAll.Add(item);
                }
            }
            catch (Exception ex)
            {
        ...
        private void con_click(object sender, EventArgs e)
        {
            ...
            if (str == "删除")
            {
                ...
                else
                {
                    ...
                    {
                        if (item.item_subno.Contains(lbl.Text) || item.item_subname.Contains(lbl.Text))
                        {
                            goodsList.Add(item);
                        }
                    }
        ...
        public Model.bi_t_item_info Select()
        {
            this.ShowDialog();
            return item;
        }

        private Model.bi_t_item_info item = null;
        ...
        private class CellInfo
        {
            ...
            public Model.bi_t_item_info item;
        }

        List<CellInfo> pageInfo = new List<CellInfo>();

        private void ShowCus(List<Model.bi_t_item_info> lst, Control par, ref int pageIndex, Graphics g)
        {
            int bound_left = 0;
            int w = 100;
            int h = 80;
            ...
            for (int i = 0; i < rowNum; i++)
            {
                for (int j = 0; j < colNum; j++)
                {
                    ...
                    else
                    {
                        ...
                        g.DrawString(item.item_subno + "\r\n" + item.item_name, new Font("微软雅黑", 12), Brushes.Black,
                            rec, sf);
                        index++;
                    }
                }
        ...
        private void panel2_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                this.ShowCus(goodsList, panel2, ref pageIndex, e.Graphics);
            }
        }

        private void ChooseCus_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void ChooseCus_Paint(object sender, PaintEventArgs e)
        {
