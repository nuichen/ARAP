...
namespace IvyFront
{
    public class Par
    {
        public string Read(string class_name, string par_name)
        {
            ...
            string[] lines = System.IO.File.ReadAllLines(file, Encoding.GetEncoding("gb2312"));
            if (lines == null)
            {
                return "";
            }
        ...
        public void Write(string class_name, string par_name, string par_value)
        {
            string file = Program.path + "\\setting\\par.ini";
            if (System.IO.File.Exists(file) == false)
            {
